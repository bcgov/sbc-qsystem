...
public class DistributionJobDayUsers extends AFormirovator {
    ...
    public Response getDialog(String driverClassName, String url, String username, String password,
        ...
        Response result = getDialog(
            "/ru/apertum/qsystem/reports/web/get_date_distribution_users.html",
        ...
        try (final Connection conn = getConnection(driverClassName, url, username, password,
            ...
            while (rs.next()) {
                users_select.append("<option value=").append(rs.getLong(1)).append(">")
        ...
        try {
            result.setData(new String(result.getData(), "UTF-8")
                .replaceFirst("#DATA_FOR_TITLE#",
                    "Распределение нагрузки внутри дня для пользователя:")
                .replaceFirst("#DATA_FOR_USERS#", users_select.toString()).getBytes("UTF-8"));
