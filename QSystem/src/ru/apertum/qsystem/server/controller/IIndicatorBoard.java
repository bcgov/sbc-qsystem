...
/**
 * Интерфейс событий вывода информации: зазываем, обрабатываем, освобождаем Имеет методы для
 * конфигурирования.
 *
...
public interface IIndicatorBoard extends IExtra {
    ...
    /**
     * На табло по определенному адресу должен замигать номер вызываемого клиента. При этом он
     * должен подняться на верх.
     *
    ...
    /**
     * Событие для главного табло в моменте встатия клиента в очередь. Появилось при необходимости
     * обносить таблицу ближайших в момент получения талона. Объявлен как дефолтный что бы старые
     * плагины не отвалились. Надеюсь, не отвалятся.
     *
     * @param customer Клиент, который был вызван
     */
