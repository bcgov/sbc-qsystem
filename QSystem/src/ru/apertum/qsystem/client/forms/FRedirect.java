...
/**
 * Диалог получения названия услуги для переадресации и сопутствующих параметров. Created on 29
 * Сентябрь 2008 г., 10:30 Класс диалоговой формы с выбором требуемой услуги из выпадающего списка.
...
public class FRedirect extends JDialog {
    ...
    private static String getLocaleMessage(String key) {
        if (localeMap == null) {
            localeMap = Application.getInstance(QSystem.class).getContext()
                .getResourceMap(FRedirect.class);
        }
     ...
     */
    public static FRedirect getService(INetProperty netProperty, JFrame owner, String tempComments,
        ...
        servicesForm.setLocation(
            Math.round(owner.getLocation().x + owner.getWidth() / 2 - servicesForm.getWidth() / 2),
            Math.round(
                owner.getLocation().y + owner.getHeight() / 2 - servicesForm.getHeight() / 2));
        servicesForm.textAreaTempComments
            .setText((tempComments == null || tempComments.isEmpty() ? ""
    ...
    private JTreeComboBox init(INetProperty netProperty, JFrame owner) {
        ...
        list.stream().forEach((ser) -> {
            ...
            ser.getChildren().stream().filter((che) -> (!goodStatus.contains(che.getStatus())))
                .forEach((che) -> {
                    del.add(che);
                });
        ...
        final JTreeComboBox cbServs = new JTreeComboBox(
            new ServiceTreeModel()); // это две связанные строчки
        // cmp.setSize(250, 24);
    ...
    public String getTempComments() {
        return (checkBoxBack.isSelected() ? checkBoxBack.getText()
            : (checkBoxBack.isVisible() ? "" : getLocaleMessage("redirect.message"))) + "\n"
    ...
    private void initComponents() {
        ...
        jPanel1Layout.setVerticalGroup(
            jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                .addComponent(jScrollPane1, javax.swing.GroupLayout.DEFAULT_SIZE, 88,
                    Short.MAX_VALUE)
        );
